...
JNIEXPORT jint JNICALL Java_edu_dhbw_andar_ARToolkit_artoolkit_1detectmarkers(
	...
	if (arDetectMarker(dataPtr, thresh, &marker_info, &marker_num) < 0) {
		...
		jclass exc = (*env)->FindClass(env,
				"edu/dhbw/andar/exceptions/AndARException");
		if (exc != NULL)
	...
	for( i = 0; i < marker_num; i++ ) {
		/*
		 printf("cf = %g\n", wmarker_info[i].cf);
		 */
		int pawnId = marker_info[i].id;
		float x = marker_info[i].pos[1]/320.0;
		float y = marker_info[i].pos[0]/480.0;

		__android_log_print(ANDROID_LOG_INFO,"BANANA","cf[%d] = %g id[%d]=%d",i,marker_info[i].cf,i,marker_info[i].id);
		__android_log_print(ANDROID_LOG_INFO,"BANANA","============================================================");
		__android_log_print(ANDROID_LOG_INFO,"BANANA-ID","wmarker_info[%d].id = %d;",i,marker_info[i].id);
		__android_log_print(ANDROID_LOG_INFO,"BANANA","wmarker_info[%d].POSITION: Y = %f; X = %f",i,marker_info[i].pos[0], marker_info[i].pos[1]);
		__android_log_print(ANDROID_LOG_INFO,"BANANA","wmarker_info[%d].RELATIVE_POSITION: Y = %f; X = %f",i,marker_info[i].pos[0]/480.0, marker_info[i].pos[1]/320.0);
		__android_log_print(ANDROID_LOG_INFO,"BANANA","wmarker_info[%d].vertex[0][0] (Y0) = %f",i,marker_info[i].vertex[0][0]);
		__android_log_print(ANDROID_LOG_INFO,"BANANA","wmarker_info[%d].vertex[0][1] (X0) = %f",i,marker_info[i].vertex[0][1]);
		__android_log_print(ANDROID_LOG_INFO,"BANANA","wmarker_info[%d].vertex[1][0] (Y1) = %f",i,marker_info[i].vertex[1][0]);
		__android_log_print(ANDROID_LOG_INFO,"BANANA","wmarker_info[%d].vertex[1][1] (X1) = %f",i,marker_info[i].vertex[1][1]);
		__android_log_print(ANDROID_LOG_INFO,"BANANA","wmarker_info[%d].vertex[2][0] (Y2) = %f",i,marker_info[i].vertex[2][0]);
		__android_log_print(ANDROID_LOG_INFO,"BANANA","wmarker_info[%d].vertex[2][1] (X2) = %f",i,marker_info[i].vertex[2][1]);
		__android_log_print(ANDROID_LOG_INFO,"BANANA","wmarker_info[%d].vertex[3][0] (Y3) = %f",i,marker_info[i].vertex[3][0]);
		__android_log_print(ANDROID_LOG_INFO,"BANANA","wmarker_info[%d].vertex[3][1] (X3) = %f",i,marker_info[i].vertex[3][1]);
		jint jPawnId = (jint) pawnId;
		jfloat jx = (jfloat) x;
		jfloat jy = (jfloat) y;
		if (g_jvm != NULL) {

			__android_log_print(ANDROID_LOG_INFO,"BANANA","JVM environment not null");
			JNIEnv *env;
			(*g_jvm)->GetEnv(g_jvm, (void**)&env, JNI_VERSION_1_4);
			//            (*env)-> MonitorEnter(env,g_obj);

			__android_log_print(ANDROID_LOG_INFO,"BANANA","got environment");
			__android_log_print(ANDROID_LOG_INFO,"BANANA","environment == null: %s",(env == NULL)?"true":"false");
			__android_log_print(ANDROID_LOG_INFO,"BANANA","g_obj == null: %s",(g_obj == NULL)?"true":"false");
			__android_log_print(ANDROID_LOG_INFO,"BANANA","g_callback == null: %s",(g_callback == NULL)?"true":"false");
			__android_log_print(ANDROID_LOG_INFO,"BANANA","calling method");
			__android_log_print(ANDROID_LOG_INFO,"BANANA","pawnId = %d x = %f y = %f", pawnId, x, y);

			(*env)->CallVoidMethod(env, g_obj, g_callback, jPawnId, jx, jy);

			__android_log_print(ANDROID_LOG_INFO,"BANANA","method called");
	...
	while (list_iterator_hasnext(&objects)) { /* tell whether more values available */
		...
		if (visibleField == NULL) {
			...
			if (arObjectClass != NULL) {
				visibleField = (*env)->GetFieldID(env, arObjectClass,
						"visible", "Z");//Z means boolean
			}
		}
		...
		if (visibleField == NULL || glMatrixField == NULL || transMatField
				== NULL) {
		...
		double* transMat = (*env)->GetDoubleArrayElements(env,
				transMatArrayObj, JNI_FALSE);
		if (transMat == NULL) {
#ifdef DEBUG_LOGGING
			__android_log_write(ANDROID_LOG_INFO,"AR native","failed to fetch the matrix arrays");
#endif
		...
		if (curObject->contF == 0) {
			...
			arGetTransMatCont(&marker_info[k], transMat,
					curObject->marker_center, curObject->marker_width, transMat);
		}
		curObject->contF = 1;
#ifdef DEBUG_LOGGING
 ...
 * Signature: ([D[D)I
 */
JNIEXPORT jint JNICALL Java_edu_dhbw_andar_ARToolkit_arUtilMatInv(JNIEnv *env,
		jclass this, jdoubleArray mat1, jdoubleArray mat2) {
	double *mat1Ptr;
	double *mat2Ptr;
	int retval;
 ...
 * Signature: ([D[D[D)I
 */
JNIEXPORT jint JNICALL Java_edu_dhbw_andar_ARToolkit_arUtilMatMul(JNIEnv *env,
		jclass this, jdoubleArray mat1, jdoubleArray mat2, jdoubleArray result) {
	double *mat1Ptr;
	double *mat2Ptr;
	double *resPtr;
	int retval;
